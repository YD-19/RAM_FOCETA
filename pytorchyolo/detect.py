...
def detect_directory(model_path, weights_path, img_path, classes, output_path,
                     batch_size=8, img_size=416, n_cpu=8, conf_thres=0.3, nms_thres=0.3):
...
def _draw_and_save_output_images(img_detections, imgs, img_size, output_path, classes):
    ...
    for (image_path, detections) in zip(imgs, img_detections):
        ...
        _draw_and_save_output_image(
            image_path, to_cpu(detections), img_size, output_path, classes)
...
def run():
    ...
    parser.add_argument("-m", "--model", type=str, default="data/yolov3.cfg", help="Path to model definition file (.cfg)")
    parser.add_argument("-w", "--weights", type=str, default="data/yolov3_best.pth", help="Path to weights or checkpoint file (.weights or .pth)")
    parser.add_argument("-i", "--images", type=str, default="data/FOCETA/ae_samples", help="Path to directory with images to inference")
    parser.add_argument("-c", "--classes", type=str, default="data/FOCETA/classes.txt", help="Path to classes label file (.names)")
    parser.add_argument("-o", "--output", type=str, default="aeoutput", help="Path to output directory")
